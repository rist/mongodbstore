...
function Store(host, port, dbName, options) {
    ...
    this.defineEntity = function(type) {
        ...
        if (!ctor) {
            ...
            ctor.map = bindArguments(map, type);
        }
        return ctor;
    };
    ...
    function map(type, mapFunction) {
        return new MapReduce(type, mapFunction);
    }
    ...
    function Collection(type, options) {
        ...
        this.mapReduce = function(mapFunction, reduceFunction){
            //TODO add query
            var results = {};
            var cursor = collection.mapReduce(mapFunction.toString(), reduceFunction.toString(), null, null).results();
            while(cursor.hasNext()){
    ...
    function MapReduce(type, mapFunction) {
        ...
        this.select = function(property) {
            return getCollection(type).mapReduce(this.mapFunction, this.reduceFunction);
        };
